...
function StatefulAppForm() {
	...
	return (
		<div className='col'>
						...
						{({ insert, remove, push }) => (
							<div>
								...
								values.application.envs.map((env, index) => (
									<div className='row my-1' key={index}>
										<div className="col-2">
											<label className="row pr-1" htmlFor={`application.volumeMounts.${index}.name`}>Name:</label>
											<Field className="row w-100" 
												name={`application.volumeMounts.${index}.name`}
												placeholder="data"
												type="text"
											/>
											<ErrorMessage
										...
										<div className="col-4">
                                        <label className="row pr-1" htmlFor={`application.volumeMounts.${index}.name`}>Mount Path:</label>
											<Field className="row w-100" 
												name={`application.volumeMounts.${index}.mountPath`}
												placeholder="MY-VALUE"
												type="text"
											/>
											<ErrorMessage
                                        ...
                                        <div className="col-3">
                                        <label className="row pr-1" htmlFor={`application.volumeMounts.${index}.name`}>Storage Class:</label>
											<Field className="row w-100" 
												name={`application.volumeMounts.${index}.storageClassName`}
												placeholder="data"
												type="text"
											/>
											<ErrorMessage
										...
										<div className="col-2">
                                        <label className="row pr-1" htmlFor={`application.volumeMounts.${index}.name`}>Storage Size:</label>
											<Field className="row  w-100" 
												name={`application.volumeMounts.${index}.size`}
												placeholder="MY-VALUE"
												type="text"
											/>
											<ErrorMessage
			...
			<StatefulAppResult app={app.application}>{" "}</StatefulAppResult>
		</div>
	);
}
